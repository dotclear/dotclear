...
class Categories implements CategoriesInterface
{
    ...
    public function __construct(
        ...
        $this->table = $this->core->db()->con()->prefix() . self::CATEGORY_TABLE_NAME;
    }

    public function createFromBlog(string $blog_id): CategoriesInterface
    {
        return new self($this->core, $blog_id);
    }
    ...
    {
        return new MetaRecord($this->core->db()->con()->select(
            'SELECT C1.' . $this->f_id . ' ' . $this->getFields($fields, 'C1.') . ' FROM ' . $this->table . ' C1, ' . $this->table . ' C2 ' . 'WHERE C2.' . $this->f_id . ' = ' . $id . ' AND C1.' . $this->f_left . ' < C2.' . $this->f_left . ' AND C1.' . $this->f_right . ' > C2.' . $this->f_right . ' ' . $this->getCondition('AND', 'C2.') . $this->getCondition('AND', 'C1.') . 'ORDER BY C1.' . $this->f_left . ' DESC ' . $this->core->db()->con()->limit(1)
        ));
    }
    ...
    public function deleteNode(int $node, bool $keep_children = true): void
    {
        ...
        try {
            ...
            if ($keep_children) {
                $this->core->db()->con()->execute('DELETE FROM ' . $this->table . ' WHERE ' . $this->f_id . ' = ' . $node);
                ...
                $this->core->db()->con()->execute('DELETE FROM ' . $this->table . ' WHERE ' . $this->f_left . ' BETWEEN ' . $node_left . ' AND ' . $node_right);
    ...
    public function setNodeParent(int $node, int $target = 0): void
    {
        ...
        if ($target_left < $node_left && $target_right > $node_right && $target_level < $node_level - 1) {
            $offset = (int) ((((float) $target_right - (float) $node_right - (float) $node_level + (float) $target_level) / 2.0) * 2.0) + $node_level - $target_level - 1;

            $sql = 'UPDATE ' . $this->table . ' SET ' . $this->f_right . ' = CASE ' . 'WHEN ' . $this->f_right . ' BETWEEN ' . ($node_right + 1) . ' AND ' . ($target_right - 1) . ' THEN ' . $this->f_right . '-(' . ($node_right - $node_left + 1) . ') ' . 'WHEN ' . $this->f_left . ' BETWEEN ' . $node_left . ' AND ' . $node_right . ' THEN ' . $this->f_right . '+' . $offset . ' ELSE ' . $this->f_right . ' END, ' . $this->f_left . ' = CASE ' . 'WHEN ' . $this->f_left . ' BETWEEN ' . ($node_right + 1) . ' AND ' . ($target_right - 1) . ' THEN ' . $this->f_left . '-(' . ($node_right - $node_left + 1) . ') ' . 'WHEN ' . $this->f_left . ' BETWEEN ' . $node_left . ' AND ' . $node_right . ' THEN ' . $this->f_left . '+' . $offset . ' ELSE ' . $this->f_left . ' END WHERE ' . $this->f_left . ' BETWEEN ' . ($target_left + 1) . ' AND ' . ($target_right - 1) . '';
    ...
    {
        return ' ' . $start . ' ' . $prefix . "blog_id = '" . $this->core->db()->con()->escapeStr($this->blog_id) . "' ";
    }
