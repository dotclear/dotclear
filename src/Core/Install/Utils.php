...
class Utils
{
    ...
    public static function blogDefaults(?array $defaults = null): void
    {
        if (!is_array($defaults)) {
            $defaults = [
                ['allow_comments', BlogWorkspace::NS_BOOL, true,
                    'Allow comments on blog', ],
                ['allow_trackbacks', BlogWorkspace::NS_BOOL, true,
                    'Allow trackbacks on blog', ],
                ['blog_timezone', BlogWorkspace::NS_STRING, 'Europe/London',
                    'Blog timezone', ],
                ['comments_nofollow', BlogWorkspace::NS_BOOL, true,
                    'Add rel="nofollow" to comments URLs', ],
                ['comments_pub', BlogWorkspace::NS_BOOL, true,
                    'Publish comments immediately', ],
                ['comments_ttl', BlogWorkspace::NS_INT, 0,
                    'Number of days to keep comments open (0 means no ttl)', ],
                ['copyright_notice', BlogWorkspace::NS_STRING, '',
                    'Copyright notice (simple text)', ],
                ['date_format', BlogWorkspace::NS_STRING, '%A, %B %e %Y',
                    'Date format. See PHP strftime function for patterns', ],
                ['editor', BlogWorkspace::NS_STRING, '',
                    'Person responsible of the content', ],
                ['enable_html_filter', BlogWorkspace::NS_BOOL, 0,
                    'Enable HTML filter', ],
                ['lang', BlogWorkspace::NS_STRING, 'en',
                    'Default blog language', ],
                ['media_exclusion', BlogWorkspace::NS_STRING, '/\.(phps?|pht(ml)?|phl|phar|.?html?|xml|js|htaccess)[0-9]*$/i',
                    'File name exclusion pattern in media manager. (PCRE value)', ],
                ['media_img_m_size', BlogWorkspace::NS_INT, 448,
                    'Image medium size in media manager', ],
                ['media_img_s_size', BlogWorkspace::NS_INT, 240,
                    'Image small size in media manager', ],
                ['media_img_t_size', BlogWorkspace::NS_INT, 100,
                    'Image thumbnail size in media manager', ],
                ['media_img_title_pattern', BlogWorkspace::NS_STRING, 'Title ;; Date(%b %Y) ;; separator(, )',
                    'Pattern to set image title when you insert it in a post', ],
                ['media_video_width', BlogWorkspace::NS_INT, 400,
                    'Video width in media manager', ],
                ['media_video_height', BlogWorkspace::NS_INT, 300,
                    'Video height in media manager', ],
                ['nb_post_for_home', BlogWorkspace::NS_INT, 20,
                    'Number of entries on first home page', ],
                ['nb_post_per_page', BlogWorkspace::NS_INT, 20,
                    'Number of entries on home pages and category pages', ],
                ['nb_post_per_feed', BlogWorkspace::NS_INT, 20,
                    'Number of entries on feeds', ],
                ['nb_comment_per_feed', BlogWorkspace::NS_INT, 20,
                    'Number of comments on feeds', ],
                ['post_url_format', BlogWorkspace::NS_STRING, '{y}/{m}/{d}/{t}',
                    'Post URL format. {y}: year, {m}: month, {d}: day, {id}: post id, {t}: entry title', ],
                ['public_path', BlogWorkspace::NS_STRING, 'public',
                    'Path to public directory, begins with a / for a full system path', ],
                ['public_url', BlogWorkspace::NS_STRING, '/public',
                    'URL to public directory', ],
                ['robots_policy', BlogWorkspace::NS_STRING, 'INDEX,FOLLOW',
                    'Search engines robots policy', ],
                ['short_feed_items', BlogWorkspace::NS_BOOL, false,
                    'Display short feed items', ],
                ['theme', BlogWorkspace::NS_STRING, DC_DEFAULT_THEME,
                    'Blog theme', ],
                ['themes_path', BlogWorkspace::NS_STRING, 'themes',
                    'Themes root path', ],
                ['themes_url', BlogWorkspace::NS_STRING, '/themes',
                    'Themes root URL', ],
                ['time_format', BlogWorkspace::NS_STRING, '%H:%M',
                    'Time format. See PHP strftime function for patterns', ],
                ['tpl_allow_php', BlogWorkspace::NS_BOOL, false,
                    'Allow PHP code in templates', ],
                ['tpl_use_cache', BlogWorkspace::NS_BOOL, true,
                    'Use template caching', ],
                ['trackbacks_pub', BlogWorkspace::NS_BOOL, true,
                    'Publish trackbacks immediately', ],
                ['trackbacks_ttl', BlogWorkspace::NS_INT, 0,
                    'Number of days to keep trackbacks open (0 means no ttl)', ],
                ['url_scan', BlogWorkspace::NS_STRING, 'query_string',
                    'URL handle mode (path_info or query_string)', ],
                ['no_public_css', BlogWorkspace::NS_BOOL, false,
                    'Don\'t use generic public.css stylesheet', ],
                ['use_smilies', BlogWorkspace::NS_BOOL, false,
                    'Show smilies on entries and comments', ],
                ['no_search', BlogWorkspace::NS_BOOL, false,
                    'Disable search', ],
                ['inc_subcats', BlogWorkspace::NS_BOOL, false,
                    'Include sub-categories in category page and category posts feed', ],
                ['wiki_comments', BlogWorkspace::NS_BOOL, false,
                    'Allow commenters to use a subset of wiki syntax', ],
                ['import_feed_url_control', BlogWorkspace::NS_BOOL, true,
                    'Control feed URL before import', ],
                ['import_feed_no_private_ip', BlogWorkspace::NS_BOOL, true,
                    'Prevent import feed from private IP', ],
                ['import_feed_ip_regexp', BlogWorkspace::NS_STRING, '',
                    'Authorize import feed only from this IP regexp', ],
                ['import_feed_port_regexp', BlogWorkspace::NS_STRING, '/^(80|443)$/',
                    'Authorize import feed only from this port regexp', ],
                ['jquery_needed', BlogWorkspace::NS_BOOL, true,
                    'Load jQuery library', ],
                ['sleepmode_timeout', BlogWorkspace::NS_INT, 31536000,
                    'Sleep mode timeout', ],
                ['store_plugin_url', BlogWorkspace::NS_STRING, 'https://update.dotaddict.org/dc2/plugins.xml',
                    'Plugins XML feed location', ],
                ['store_theme_url', BlogWorkspace::NS_STRING, 'https://update.dotaddict.org/dc2/themes.xml',
