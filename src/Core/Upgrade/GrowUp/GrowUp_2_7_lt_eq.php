...
class GrowUp_2_7_lt_eq
{
    public static function init(bool $cleanup_sessions): bool
    {
        ...
        $strReqCount = 'SELECT count(1) FROM ' . Core::con()->prefix() . dcNamespace::NS_TABLE_NAME;
        $strReqCount .= ' WHERE setting_id = \'%s\'';
        $strReqCount .= ' AND setting_ns = \'system\'';
        $strReqCount .= ' AND blog_id IS NULL';

        $strReqSelect = 'SELECT setting_value FROM ' . Core::con()->prefix() . dcNamespace::NS_TABLE_NAME;
