...
class User extends Process
{
    public static function init(): bool
    {
        ...
        Core::backend()->page_title = __('New user');

        Core::backend()->user_id          = '';
        Core::backend()->user_super       = '';
        Core::backend()->user_change_pwd  = '';
        Core::backend()->user_name        = '';
        Core::backend()->user_firstname   = '';
        ...
        Core::backend()->user_profile_mails = '';
        Core::backend()->user_profile_urls  = '';

        # Formaters combo
        Core::backend()->formaters_combo = Combos::getFormatersCombo();
        ...
        if (!empty($_REQUEST['id'])) {
            try {
                Core::backend()->rs = Core::users()->getUser($_REQUEST['id']);

                Core::backend()->user_id          = Core::backend()->rs->user_id;
                Core::backend()->user_super       = Core::backend()->rs->user_super;
                Core::backend()->user_change_pwd  = Core::backend()->rs->user_change_pwd;
                Core::backend()->user_name        = Core::backend()->rs->user_name;
                Core::backend()->user_firstname   = Core::backend()->rs->user_firstname;
                Core::backend()->user_displayname = Core::backend()->rs->user_displayname;
                Core::backend()->user_email       = Core::backend()->rs->user_email;
                Core::backend()->user_url         = Core::backend()->rs->user_url;
                Core::backend()->user_lang        = Core::backend()->rs->user_lang;
                Core::backend()->user_tz          = Core::backend()->rs->user_tz;
                Core::backend()->user_post_status = Core::backend()->rs->user_post_status;

                Core::backend()->user_options = array_merge(Core::backend()->user_options, Core::backend()->rs->options());

                $user_prefs = new dcPrefs(Core::backend()->user_id, 'profile');

                Core::backend()->user_profile_mails = $user_prefs->profile->mails;
                Core::backend()->user_profile_urls  = $user_prefs->profile->urls;
    ...
    public static function process(): bool
    {
        if (isset($_POST['user_name'])) {
            ...
            try {
                ...
                $cur->user_super       = Core::backend()->user_super = !empty($_POST['user_super']) ? 1 : 0;
                $cur->user_name        = Core::backend()->user_name = Html::escapeHTML($_POST['user_name']);
                $cur->user_firstname   = Core::backend()->user_firstname = Html::escapeHTML($_POST['user_firstname']);
                $cur->user_displayname = Core::backend()->user_displayname = Html::escapeHTML($_POST['user_displayname']);
                $cur->user_email       = Core::backend()->user_email = Html::escapeHTML($_POST['user_email']);
                $cur->user_url         = Core::backend()->user_url = Html::escapeHTML($_POST['user_url']);
                $cur->user_lang        = Core::backend()->user_lang = Html::escapeHTML($_POST['user_lang']);
                ...
                if (Core::backend()->user_id) {
                    // Update user

                    # --BEHAVIOR-- adminBeforeUserUpdate -- Cursor, string
                    Core::behavior()->callBehavior('adminBeforeUserUpdate', $cur, Core::backend()->user_id);

                    $new_id = Core::users()->updUser(Core::backend()->user_id, $cur);
                    ...
                    Core::backend()->url->redirect('admin.user', ['id' => $new_id]);
                } else {
                    // Add user

                    if (Core::users()->getUsers(['user_id' => $cur->user_id], true)->f(0) > 0) {
                    ...
                    if (!empty($_POST['saveplus'])) {
                        Core::backend()->url->redirect('admin.user');
                    } else {
                        Core::backend()->url->redirect('admin.user', ['id' => $new_id]);
                    }
                }
    ...
    public static function render(): void
    {
        ...
        echo
        '<form action="' . Core::backend()->url->get('admin.user') . '" method="post" id="user-form">' .
        '<div class="two-cols">' .
        ...
        '<p>' .
        '<label for="new_pwd" ' .
        (Core::backend()->user_id != '' ? '' : 'class="required"') . '>' .
        (Core::backend()->user_id != '' ? '' : '<abbr title="' . __('Required field') . '">*</abbr> ') .
        (Core::backend()->user_id != '' ? __('New password:') : __('Password:')) . '</label>' .
        form::password(
            'new_pwd',
            20,
            255,
            [
                'class'        => 'pw-strength',
                'extra_html'   => (Core::backend()->user_id != '' ? '' : 'aria-describedby="new_pwd_help" required placeholder="' . __('Password') . '"'),
                'autocomplete' => 'new-password', ]
        ) .
        '</p>' .
        '<p class="form-note info" id="new_pwd_help">' . __('Password must contain at least 6 characters.') . '</p>' .

        '<p><label for="new_pwd_c" ' . (Core::backend()->user_id != '' ? '' : 'class="required"') . '>' .
        (Core::backend()->user_id != '' ? '' : '<abbr title="' . __('Required field') . '">*</abbr> ') . __('Confirm password:') . '</label> ' .
        form::password(
            'new_pwd_c',
            20,
            255,
            [
                'extra_html'   => (Core::backend()->user_id != '' ? '' : 'required placeholder="' . __('Password') . '"'),
                'autocomplete' => 'new-password', ]
        ) .
        '</p>';
        ...
        form::checkbox(
            ...
            '1',
            [
                'checked'  => Core::backend()->user_super,
                'disabled' => $super_disabled,
            ]
        ) .
        ' ' . __('Super administrator') . '</label></p>' .
        ($super_disabled ? form::hidden(['user_super'], Core::backend()->user_super) : '') .
        ...
        form::combo('user_lang', Core::backend()->lang_combo, Core::backend()->user_lang, 'l10n') .
        '</p>' .

        '<p><label for="user_tz">' . __('Timezone:') . '</label> ' .
        form::combo('user_tz', Date::getZones(true, true), Core::backend()->user_tz) .
        '</p>' .

        '<h4>' . __('Edition') . '</h4>' .
        '<p><label for="user_post_format">' . __('Preferred format:') . '</label> ' .
        form::combo('user_post_format', Core::backend()->formaters_combo, Core::backend()->user_options['post_format']) .
        '</p>' .

        '<p><label for="user_post_status">' . __('Default entry status:') . '</label> ' .
        form::combo('user_post_status', Core::backend()->status_combo, Core::backend()->user_post_status) .
        '</p>' .

        '<p><label for="user_edit_size">' . __('Entry edit field height:') . '</label> ' .
        form::number('user_edit_size', 10, 999, (string) Core::backend()->user_options['edit_size']) .
        '</p>';

        # --BEHAVIOR-- adminUserForm -- MetaRecord|null
        Core::behavior()->callBehavior('adminUserForm', Core::backend()->rs ?? null);
        ...
        (Core::backend()->user_id != '' ? '' : ' <input type="submit" name="saveplus" value="' . __('Save and create another') . '" />') .
        (Core::backend()->user_id != '' ? form::hidden('id', Core::backend()->user_id) : '') .
        ...
        if (Core::backend()->user_id) {
            ...
            if (!Core::backend()->user_super) {
                echo
                '<form action="' . Core::backend()->url->get('admin.user.actions') . '" method="post">' .
                '<p><input type="submit" value="' . __('Add new permissions') . '" />' .
                form::hidden(['redir'], Core::backend()->url->get('admin.user', ['id' => Core::backend()->user_id])) .
                form::hidden(['action'], 'blogs') .
                form::hidden(['users[]'], Core::backend()->user_id) .
                Core::nonce()->getFormNonce() .
                '</p>' .
                '</form>';
                ...
                if ((is_countable($permissions) ? count($permissions) : 0) == 0) {  // @phpstan-ignore-line
                    ...
                    foreach ($permissions as $k => $v) {
                        if ((is_countable($v['p']) ? count($v['p']) : 0) > 0) {
                            echo
                            '<form action="' . Core::backend()->url->get('admin.user.actions') . '" method="post" class="perm-block">' .
                            '<p class="blog-perm">' . __('Blog:') . ' <a href="' .
                            Core::backend()->url->get('admin.blog', ['id' => Html::escapeHTML($k)]) . '">' .
                ...
                echo
                '<p>' . sprintf(__('%s is super admin (all rights on all blogs).'), '<strong>' . Core::backend()->user_id . '</strong>') . '</p>';
            }
            echo
            '</div>';
            ...
            '<p><a href="' . Core::backend()->url->get(
                'admin.posts',
                ['user_id' => Core::backend()->user_id]
            ) . '">' . __('List of posts') . '</a></p>';

            if (Core::backend()->user_email || Core::backend()->user_url) {
                echo
                '<p><a href="' . Core::backend()->url->get(
                    'admin.comments',
                    [
                        'email' => Core::backend()->user_email,
                        'site'  => Core::backend()->user_url,
                    ]
