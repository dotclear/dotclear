...
class BlogPref extends Process
{
    ...
    /** @var    string  The page action URL */
    private static string $action;

    /** @var    string  The page redirection URL */
    private static string $redir;

    /** @var    array<string,string>    Langs combo */
    private static array $lang_combo;

    /** @var    array<string,int>   Blogs status combo */
    private static array $status_combo;

    /** @var    int     Current time */
    private static int $date_now;

    /** @var    array<string,string>    Date formats combo */
    private static array $date_formats_combo;

    /** @var    array<string,string>    Time formats combo */
    private static array $time_formats_combo;

    /** @var    array<string,string>    URL scan modes */
    private static array $url_scan_combo = [
        'PATH_INFO'    => 'path_info',
        'QUERY_STRING' => 'query_string',
    ];

    /** @var    array<string,string>    Post URL combo */
    private static array $post_url_combo;

    /** @var    array<string,int>   Note title tag combo */
    private static array $note_title_tag_combo;

    /** @var    array<string,string>    Image title combo */
    private static array $img_title_combo;

    /** @var array<string,string> Image default size combo */
    private static array $img_default_size_combo;

    /** @var    array<string,string>    Image default alignment combo */
    private static array $img_default_alignment_combo;

    /** @var    array<string,string>    Image default legend and title combo */
    private static array $img_default_legend_combo;

    /** @var    array<string,string>    Robots policy options */
    private static array $robots_policy_options;

    /** @var    array<string,string>    jQuery available versions */
    private static array $jquery_versions_combo;

    /** @var    array<string,int>   Sleep mode timeout in second */
    private static array $sleepmode_timeout_combo;

    private static string $blog_id   = '';
    private static int $blog_status  = dcBlog::BLOG_OFFLINE;
    private static string $blog_name = '';
    private static string $blog_desc = '';
    private static string $blog_url  = '';
    private static dcSettings $blog_settings;
    ...
    public static function init(): bool
    {
        if (self::$standalone) {
            ...
            self::$blog_id       = dcCore::app()->blog->id;
            self::$blog_status   = (int) dcCore::app()->blog->status;
            self::$blog_name     = dcCore::app()->blog->name;
            self::$blog_desc     = dcCore::app()->blog->desc;
            self::$blog_settings = dcCore::app()->blog->settings;
            self::$blog_url      = dcCore::app()->blog->url;

            self::$action = dcCore::app()->admin->url->get('admin.blog.pref');
            self::$redir  = dcCore::app()->admin->url->get('admin.blog.pref');
        } else {
            Page::checkSuper();
            ...
            try {
                ...
                self::$blog_status   = (int) $rs->blog_status;
                self::$blog_name     = $rs->blog_name;
                self::$blog_desc     = $rs->blog_desc;
                self::$blog_settings = new dcSettings(self::$blog_id);
                self::$blog_url      = $rs->blog_url;
            } catch (Exception $e) {
                dcCore::app()->error->add($e->getMessage());
            }

            self::$action = dcCore::app()->admin->url->get('admin.blog');
            self::$redir  = dcCore::app()->admin->url->get('admin.blog', ['id' => '%s'], '&', true);
        }

        // Language codes
        self::$lang_combo = Combos::getAdminLangsCombo();

        // Status combo
        self::$status_combo = Combos::getBlogStatusescombo();

        // Date format combo
        self::$date_now = time();

        $date_formats = isset(self::$blog_settings) ? self::$blog_settings->system->date_formats : [];
        $time_formats = isset(self::$blog_settings) ? self::$blog_settings->system->time_formats : [];

        $stack = ['' => ''];
        foreach ($date_formats as $format) {
            $stack[Date::str($format, self::$date_now)] = $format;
        }
        ...
        if (!in_array(self::$blog_settings->system->post_url_format, self::$post_url_combo)) {
            self::$post_url_combo[Html::escapeHTML(self::$blog_settings->system->post_url_format)] = Html::escapeHTML(self::$blog_settings->system->post_url_format);
        }
        ...
        if (!in_array(self::$blog_settings->system->media_img_title_pattern, self::$img_title_combo)) {
            self::$img_title_combo[Html::escapeHTML(self::$blog_settings->system->media_img_title_pattern)] = Html::escapeHTML(self::$blog_settings->system->media_img_title_pattern);
        }
    ...
    {
        if (self::$blog_id && !empty($_POST) && dcCore::app()->auth->check(dcCore::app()->auth->makePermissions(
            [
                dcCore::app()->auth::PERMISSION_ADMIN,
            ]
        ), self::$blog_id)) {
            ...
            if (dcCore::app()->auth->isSuperAdmin() && in_array($_POST['blog_status'], self::$status_combo)) {
            ...
            try {
                if ($cur->blog_id != null && $cur->blog_id != self::$blog_id) {
                ...
                dcCore::app()->callBehavior('adminBeforeBlogUpdate', $cur, self::$blog_id);

                if (!preg_match('/^[a-z]{2}(-[a-z]{2})?$/', (string) $_POST['lang'])) {
                    throw new Exception(__('Invalid language code'));
                }

                dcCore::app()->updBlog(self::$blog_id, $cur);
                ...
                dcCore::app()->callBehavior('adminAfterBlogUpdate', $cur, self::$blog_id);

                if ($cur->blog_id != null && $cur->blog_id != self::$blog_id) {
                    if (self::$blog_id == dcCore::app()->blog->id) {
                        dcCore::app()->setBlog($cur->blog_id);
                        $_SESSION['sess_blog_id'] = $cur->blog_id;
                        self::$blog_settings      = dcCore::app()->blog->settings;
                    } else {
                        self::$blog_settings = new dcSettings($cur->blog_id);
                    }

                    self::$blog_id = $cur->blog_id;
                }

                self::$blog_settings->system->put('editor', $_POST['editor']);
                self::$blog_settings->system->put('copyright_notice', $_POST['copyright_notice']);
                self::$blog_settings->system->put('post_url_format', $_POST['post_url_format']);
                self::$blog_settings->system->put('lang', $_POST['lang']);
                self::$blog_settings->system->put('blog_timezone', $_POST['blog_timezone']);
                self::$blog_settings->system->put('date_format', $_POST['date_format']);
                self::$blog_settings->system->put('time_format', $_POST['time_format']);
                self::$blog_settings->system->put('comments_ttl', abs((int) $_POST['comments_ttl']));
                self::$blog_settings->system->put('trackbacks_ttl', abs((int) $_POST['trackbacks_ttl']));
                self::$blog_settings->system->put('allow_comments', !empty($_POST['allow_comments']));
                self::$blog_settings->system->put('allow_trackbacks', !empty($_POST['allow_trackbacks']));
                self::$blog_settings->system->put('comments_pub', empty($_POST['comments_pub']));
                self::$blog_settings->system->put('trackbacks_pub', empty($_POST['trackbacks_pub']));
                self::$blog_settings->system->put('comments_nofollow', !empty($_POST['comments_nofollow']));
                self::$blog_settings->system->put('wiki_comments', !empty($_POST['wiki_comments']));
                self::$blog_settings->system->put('comment_preview_optional', !empty($_POST['comment_preview_optional']));
                self::$blog_settings->system->put('note_title_tag', $_POST['note_title_tag']);
                self::$blog_settings->system->put('nb_post_for_home', $nb_post_for_home);
                self::$blog_settings->system->put('nb_post_per_page', $nb_post_per_page);
                self::$blog_settings->system->put('no_public_css', !empty($_POST['no_public_css']));
                self::$blog_settings->system->put('use_smilies', !empty($_POST['use_smilies']));
                self::$blog_settings->system->put('no_search', !empty($_POST['no_search']));
                self::$blog_settings->system->put('inc_subcats', !empty($_POST['inc_subcats']));
                self::$blog_settings->system->put('media_img_t_size', $media_img_t_size);
                self::$blog_settings->system->put('media_img_s_size', $media_img_s_size);
                self::$blog_settings->system->put('media_img_m_size', $media_img_m_size);
                self::$blog_settings->system->put('media_video_width', $media_video_width);
                self::$blog_settings->system->put('media_video_height', $media_video_height);
                self::$blog_settings->system->put('media_img_title_pattern', $_POST['media_img_title_pattern']);
                self::$blog_settings->system->put('media_img_use_dto_first', !empty($_POST['media_img_use_dto_first']));
                self::$blog_settings->system->put('media_img_no_date_alone', !empty($_POST['media_img_no_date_alone']));
                self::$blog_settings->system->put('media_img_default_size', $_POST['media_img_default_size']);
                self::$blog_settings->system->put('media_img_default_alignment', $_POST['media_img_default_alignment']);
                self::$blog_settings->system->put('media_img_default_link', !empty($_POST['media_img_default_link']));
                self::$blog_settings->system->put('media_img_default_legend', $_POST['media_img_default_legend']);
                self::$blog_settings->system->put('nb_post_per_feed', $nb_post_per_feed);
                self::$blog_settings->system->put('nb_comment_per_feed', $nb_comment_per_feed);
                self::$blog_settings->system->put('short_feed_items', !empty($_POST['short_feed_items']));
                if (isset($_POST['robots_policy'])) {
                    self::$blog_settings->system->put('robots_policy', $_POST['robots_policy']);
                }
                self::$blog_settings->system->put('jquery_needed', !empty($_POST['jquery_needed']));
                self::$blog_settings->system->put('jquery_version', $_POST['jquery_version']);
                self::$blog_settings->system->put('prevents_clickjacking', !empty($_POST['prevents_clickjacking']));
                self::$blog_settings->system->put('static_home', !empty($_POST['static_home']));
                self::$blog_settings->system->put('static_home_url', $_POST['static_home_url']);

                self::$blog_settings->system->put('sleepmode_timeout', $_POST['sleepmode_timeout']);

                # --BEHAVIOR-- adminBeforeBlogSettingsUpdate -- dcSettings
                dcCore::app()->callBehavior('adminBeforeBlogSettingsUpdate', self::$blog_settings);

                if (dcCore::app()->auth->isSuperAdmin() && in_array($_POST['url_scan'], self::$url_scan_combo)) {
                    self::$blog_settings->system->put('url_scan', $_POST['url_scan']);
                }
    ...
    public static function render(): void
    {
        ...
        if (self::$blog_id) {
            ...
            '<form action="' . self::$action . '" method="post" id="blog-form">' .
            ...
            form::field(
                ...
                255,
                [
                    'default'    => Html::escapeHTML(self::$blog_name),
                    'extra_html' => 'required placeholder="' . __('Blog name') . '" lang="' . self::$blog_settings->system->lang . '" spellcheck="true"',
                ]
            ) . '</p>' .
            ...
            form::textarea(
                ...
                5,
                [
                    'default'    => Html::escapeHTML(self::$blog_desc),
                    'extra_html' => 'lang="' . self::$blog_settings->system->lang . '" spellcheck="true"',
                ]
            ) . '</p>';
            ...
            if (dcCore::app()->auth->isSuperAdmin()) {
                ...
                form::combo('blog_status', self::$status_combo, self::$blog_status) . '</p>';
            } else {
                /*
                 ...
                 */
                echo
                form::hidden('blog_id', Html::escapeHTML(self::$blog_id)) .
                form::hidden('blog_url', Html::escapeHTML(self::$blog_url));
            }
            ...
            form::field('editor', 30, 255, Html::escapeHTML(self::$blog_settings->system->editor)) .
            '</p>' .
            '<p><label for="lang">' . __('Default language:') . '</label>' .
            form::combo('lang', self::$lang_combo, self::$blog_settings->system->lang, 'l10n') .
            '</p>' .
            '<p><label for="blog_timezone">' . __('Blog timezone:') . '</label>' .
            form::combo('blog_timezone', Date::getZones(true, true), Html::escapeHTML(self::$blog_settings->system->blog_timezone)) .
            '</p>' .
            ...
            form::field(
                ...
                255,
                [
                    'default'    => Html::escapeHTML(self::$blog_settings->system->copyright_notice),
                    'extra_html' => 'lang="' . self::$blog_settings->system->lang . '" spellcheck="true"',
                ]
            ) .
            '</p>' .
            ...
            form::checkbox('allow_comments', '1', self::$blog_settings->system->allow_comments) .
            __('Accept comments') . '</label></p>' .
            '<p><label for="comments_pub" class="classic">' .
            form::checkbox('comments_pub', '1', !self::$blog_settings->system->comments_pub) .
            ...
            '<p><label for="comments_ttl" class="classic">' . sprintf(
                ...
                form::number(
                    ...
                    [
                        'min'        => 0,
                        'max'        => 999,
                        'default'    => self::$blog_settings->system->comments_ttl,
            ...
            form::checkbox('wiki_comments', '1', self::$blog_settings->system->wiki_comments) .
            __('Wiki syntax for comments') . '</label></p>' .
            '<p><label for="comment_preview_optional" class="classic">' .
            form::checkbox('comment_preview_optional', '1', self::$blog_settings->system->comment_preview_optional) .
            __('Preview of comment before submit is not mandatory') . '</label></p>' .
            '</div>' .

            '<div class="col">' .
            '<p><label for="allow_trackbacks" class="classic">' .
            form::checkbox('allow_trackbacks', '1', self::$blog_settings->system->allow_trackbacks) .
            __('Accept trackbacks') . '</label></p>' .
            '<p><label for="trackbacks_pub" class="classic">' .
            form::checkbox('trackbacks_pub', '1', !self::$blog_settings->system->trackbacks_pub) .
            ...
            '<p><label for="trackbacks_ttl" class="classic">' . sprintf(
                ...
                form::number(
                    ...
                    [
                        'min'        => 0,
                        'max'        => 999,
                        'default'    => self::$blog_settings->system->trackbacks_ttl,
            ...
            form::checkbox('comments_nofollow', '1', self::$blog_settings->system->comments_nofollow) .
            ...
            form::combo('sleepmode_timeout', self::$sleepmode_timeout_combo, self::$blog_settings->system->sleepmode_timeout) .
            '</p>' .
            ...
            form::field('date_format', 30, 255, Html::escapeHTML(self::$blog_settings->system->date_format), '', '', false, 'aria-describedby="date_format_help"') .
            form::combo('date_format_select', self::$date_formats_combo, ['extra_html' => 'title="' . __('Pattern of date') . '"']) .
            '</p>' .
            '<p class="chosen form-note" id="date_format_help">' . __('Sample:') . ' ' . Date::str(Html::escapeHTML(self::$blog_settings->system->date_format)) . '</p>' .

            '<p><label for="time_format">' . __('Time format:') . '</label>' .
            form::field('time_format', 30, 255, Html::escapeHTML(self::$blog_settings->system->time_format), '', '', false, 'aria-describedby="time_format_help"') .
            form::combo('time_format_select', self::$time_formats_combo, ['extra_html' => 'title="' . __('Pattern of time') . '"']) .
            '</p>' .
            '<p class="chosen form-note" id="time_format_help">' . __('Sample:') . ' ' . Date::str(Html::escapeHTML(self::$blog_settings->system->time_format)) . '</p>' .

            '<p><label for="no_public_css" class="classic">' .
            form::checkbox('no_public_css', '1', self::$blog_settings->system->no_public_css) .
            __('Don\'t load standard stylesheet (used for media alignement)') . '</label></p>' .

            '<p><label for="use_smilies" class="classic">' .
            form::checkbox('use_smilies', '1', self::$blog_settings->system->use_smilies) .
            __('Display smilies on entries and comments') . '</label></p>' .

            '<p><label for="no_search" class="classic">' .
            form::checkbox('no_search', '1', self::$blog_settings->system->no_search) .
            ...
            '<p><label for="nb_post_for_home" class="classic">' . sprintf(
                ...
                form::number(
                    ...
                    [
                        'min'     => 1,
                        'max'     => 999,
                        'default' => self::$blog_settings->system->nb_post_for_home, ]
                )
            ) .
            '</label></p>' .
            ...
            '<p><label for="nb_post_per_page" class="classic">' . sprintf(
                ...
                form::number(
                    ...
                    [
                        'min'     => 1,
                        'max'     => 999,
                        'default' => self::$blog_settings->system->nb_post_per_page, ]
                )
            ) .
            '</label></p>' .
            ...
            '<p><label for="nb_post_per_feed" class="classic">' . sprintf(
                ...
                form::number(
                    ...
                    [
                        'min'     => 1,
                        'max'     => 999,
                        'default' => self::$blog_settings->system->nb_post_per_feed, ]
                )
            ) .
            '</label></p>' .
            ...
            '<p><label for="nb_comment_per_feed" class="classic">' . sprintf(
                ...
                form::number(
                    ...
                    [
                        'min'     => 1,
                        'max'     => 999,
                        'default' => self::$blog_settings->system->nb_comment_per_feed, ]
                )
            ) .
            '</label></p>' .

            '<p><label for="short_feed_items" class="classic">' .
            form::checkbox('short_feed_items', '1', self::$blog_settings->system->short_feed_items) .
            __('Truncate feeds') . '</label></p>' .

            '<p><label for="inc_subcats" class="classic">' .
            form::checkbox('inc_subcats', '1', self::$blog_settings->system->inc_subcats) .
            ...
            form::checkbox('static_home', '1', self::$blog_settings->system->static_home) .
            ...
            form::field('static_home_url', 30, 255, Html::escapeHTML(self::$blog_settings->system->static_home_url), '', '', false, 'aria-describedby="static_home_url_help"') .
            ...
            form::number('media_img_t_size', [
                'min'     => -1,
                'max'     => 999,
                'default' => self::$blog_settings->system->media_img_t_size,
            ]) .
            '</p>' .
            ...
            form::number('media_img_s_size', [
                'min'     => -1,
                'max'     => 999,
                'default' => self::$blog_settings->system->media_img_s_size,
            ]) .
            '</p>' .
            ...
            form::number('media_img_m_size', [
                'min'     => -1,
                'max'     => 999,
                'default' => self::$blog_settings->system->media_img_m_size,
            ]) .
            '</p>' .
            ...
            form::number('media_video_width', [
                'min'     => -1,
                'max'     => 999,
                'default' => self::$blog_settings->system->media_video_width,
            ]) .
            '</p>' .
            ...
            form::number('media_video_height', [
                'min'     => -1,
                'max'     => 999,
                'default' => self::$blog_settings->system->media_video_height,
            ]) .
            '</p>' .
            '</div>' .
            ...
            form::combo('media_img_title_pattern', self::$img_title_combo, Html::escapeHTML(self::$blog_settings->system->media_img_title_pattern)) . '</p>' .
            '<p><label for="media_img_use_dto_first" class="classic">' .
            form::checkbox('media_img_use_dto_first', '1', self::$blog_settings->system->media_img_use_dto_first) .
            __('Use original media date if possible') . '</label></p>' .
            '<p><label for="media_img_no_date_alone" class="classic">' .
            form::checkbox('media_img_no_date_alone', '1', self::$blog_settings->system->media_img_no_date_alone, '', '', false, 'aria-describedby="media_img_no_date_alone_help"') .
            ...
            form::combo(
                ...
                self::$img_default_size_combo,
                (Html::escapeHTML(self::$blog_settings->system->media_img_default_size) != '' ? Html::escapeHTML(self::$blog_settings->system->media_img_default_size) : 'm')
            ) .
            '</p>' .
            '<p class="field"><label for="media_img_default_alignment">' . __('Image alignment:') . '</label>' .
            form::combo('media_img_default_alignment', self::$img_default_alignment_combo, Html::escapeHTML(self::$blog_settings->system->media_img_default_alignment)) .
            '</p>' .
            '<p><label for="media_img_default_link">' .
            form::checkbox('media_img_default_link', '1', self::$blog_settings->system->media_img_default_link) .
            ...
            form::combo('media_img_default_legend', self::$img_default_legend_combo, Html::escapeHTML(self::$blog_settings->system->media_img_default_legend)) .
            '</p>' .
            '</div>' .
            '</div>' . '<br class="clear" />' . //Opera sucks
            ...
            if (dcCore::app()->auth->isSuperAdmin()) {
                ...
                form::field('blog_id', 30, 32, Html::escapeHTML(self::$blog_id), '', '', false, 'required placeholder="' . __('Blog ID') . '" aria-describedby="blog_id_help blog_id_warn"') . '</p>' .
                ...
                form::url('blog_url', [
                    'size'       => 50,
                    'max'        => 255,
                    'default'    => Html::escapeHTML(self::$blog_url),
                    'extra_html' => 'required placeholder="' . __('Blog URL') . '"',
                ]) .
                '</p>' .

                '<p><label for="url_scan">' . __('URL scan method:') . '</label>' .
                form::combo('url_scan', self::$url_scan_combo, self::$blog_settings->system->url_scan) . '</p>';

                try {
                    # Test URL of blog by testing it's ATOM feed
                    $file    = self::$blog_url . dcCore::app()->url->getURLFor('feed', 'atom');
                    $path    = '';
                    $status  = '404';
                    $content = '';
            ...
            form::combo('post_url_format', self::$post_url_combo, Html::escapeHTML(self::$blog_settings->system->post_url_format), '', '', false, 'aria-describedby="post_url_format_help"') .
            '</p>' .
            '<p class="chosen form-note" id="post_url_format_help">' . __('Sample:') . ' ' . dcCore::app()->blog->getPostURL('', date('Y-m-d H:i:00', self::$date_now), __('Dotclear'), 42) . '</p>' .
            '</p>' .

            '<p><label for="note_title_tag">' . __('HTML tag for the title of the notes on the blog:') . '</label>' .
            form::combo('note_title_tag', self::$note_title_tag_combo, self::$blog_settings->system->note_title_tag) .
            '</p>' .
            ...
            $i = 0;
            foreach (self::$robots_policy_options as $k => $v) {
                echo
                '<p><label for="robots_policy-' . $i . '" class="classic">' .
                form::radio(['robots_policy', 'robots_policy-' . $i], $k, self::$blog_settings->system->robots_policy == $k) . ' ' . $v . '</label></p>';
            ...
            form::checkbox('jquery_needed', '1', self::$blog_settings->system->jquery_needed) .
            ...
            form::combo('jquery_version', self::$jquery_versions_combo, self::$blog_settings->system->jquery_version) .
            ...
            form::checkbox('prevents_clickjacking', '1', self::$blog_settings->system->prevents_clickjacking) .
            ...
            dcCore::app()->callBehavior('adminBlogPreferencesFormV2', self::$blog_settings);

            echo '</div>' . // End 3rd party, aka plugins

            '<p><input type="submit" accesskey="s" value="' . __('Save') . '" />' .
            ' <input type="button" value="' . __('Cancel') . '" class="go-back reset hidden-if-no-js" />' .
            (!self::$standalone ? form::hidden('id', self::$blog_id) : '') .
            '</p>' .
            '</form>';

            if (dcCore::app()->auth->isSuperAdmin() && self::$blog_id != dcCore::app()->blog->id) {
                echo
                ...
                form::hidden(['blog_id'], self::$blog_id) .
            ...
            $blog_users = dcCore::app()->getBlogPermissions(self::$blog_id, dcCore::app()->auth->isSuperAdmin());
            ...
            if (empty($blog_users)) {
                ...
                dcUtils::lexicalKeySort($blog_users, dcUtils::ADMIN_LOCALE);

                $post_type       = dcCore::app()->getPostTypes();
                $current_blog_id = dcCore::app()->blog->id;
                if (self::$blog_id != dcCore::app()->blog->id) {
                    dcCore::app()->setBlog(self::$blog_id);
                }
                ...
                foreach ($blog_users as $k => $v) {
                    if ((is_countable($v['p']) ? count($v['p']) : 0) > 0) {
                        ...
                        if (!$v['super'] && dcCore::app()->auth->isSuperAdmin()) {
                            ...
                            form::hidden(['users[]'], $k) .
                            form::hidden(['blogs[]'], self::$blog_id) .
                            dcCore::app()->formNonce() .
                            '</p>' .
                            '</form>';
                        }
