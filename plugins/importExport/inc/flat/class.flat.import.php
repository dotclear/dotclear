			...
			while (($line = $this->getLine()) !== false)
			{
				...
				if ($last_line_name != $line->__name) {
					if (in_array($last_line_name,$constrained)) {
						# UNDEFER
						if ($this->con->driver() == 'mysql' || $this->con->driver() == 'mysqli' || $this->con->driver() == 'mysqlimb4') $this->con->execute('SET foreign_key_checks = 1');
						if ($this->con->driver() == 'pgsql') $this->con->execute('SET CONSTRAINTS ALL DEFERRED');
					}

					if (in_array($line->__name,$constrained)) {
						# DEFER
						if ($this->con->driver() == 'mysql' || $this->con->driver() == 'mysqli' || $this->con->driver() == 'mysqlimb4') $this->con->execute('SET foreign_key_checks = 0');
